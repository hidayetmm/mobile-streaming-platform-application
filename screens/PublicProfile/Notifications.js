...
export const writeMessageAsNotification = async (
  ...
  return streamByUsername(streamerUsername, access_token).then((res) => {
    if (res.data.item) {
      ...
      /* join room */
      axios.post(
        `${Config.REACT_APP_MatrixUrl}rooms/${default_matrix_room_id}/join?access_token=${access_token}`
      );

      /* send message */
      axios.put(
        `${Config.REACT_APP_MatrixUrl}rooms/${default_matrix_room_id}/send/m.room.message/${random}?access_token=${access_token}`,
        {
          msgtype: "m.text",
          body: message,
        }
